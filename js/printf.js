		...
		if (ch != '%' || format_index == format.length) {
			...
			while (specin) {
				...
				switch (ch) {
					...
					case 'f' :
						if (arg_index < argc) {
							value = parseFloat(argv[arg_index++]);
						} else {
							value = 0;
						}
						if (llen < 0) {
					...
					/*
					 * Not supported case 'f': case 'e': case 'E': case 'g':
					 * case 'G': if (arg_index < argc) { value =
					 * argv[arg_index++]; } else { value = 0.0; } output +=
					 * _dopr_fmtdouble(format.substr(p, format_index - p),
					 * value); break;
					 */
...
function _dopr_fromCharCode(code) {
	...
	if (!_dopr_fromCharCode_chars)
		_dopr_fromCharCode_chars = "\000\001\002\003\004\005\006\007\010\011\012\013\014\015\016\017\020"
