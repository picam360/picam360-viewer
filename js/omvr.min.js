...
function OMVR() {	
	...
	var self = {
		...
		animate : function(elapsedTime) {
			if(m_window_mode) {
				if(stereoEnabled) {
					var dh = stereoEnabled?m_canvas.height/2:m_canvas.height;
					var sx=0,sy=0;
					var sa = m_video.width/m_video.height;
					var da = m_canvas.width/dh;
					if(da > sa) {
						sy = (m_video.height - m_video.width/da)/2;
					} else {
						sx = (m_video.width - m_video.height*da)/2;
					}
					m_context.drawImage(m_video, sx, sy, m_video.width - sx*2, m_video.height - sy*2, 0, 0, m_canvas.width, dh);
					if(stereoEnabled) {
						m_context.drawImage(m_video, sx, sy, m_video.width - sx*2, m_video.height - sy*2, 0, dh, m_canvas.width, dh);
					}
				} else {
					//zoom ratio 1
