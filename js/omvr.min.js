...
function OMVR() {	
	...
	"    float yaw = atan(vTexCoord.y, vTexCoord.x);",

	"    float n = length(vTexCoord.xy);",

	"    float pitch = atan(n, vTexCoord.z);",

	"    float r = 1.33 * pitch / PI;",

	"    float u = r * cos(yaw) + 0.5;",

	"    float v = r * sin(yaw) + 0.5;",
	...
	var self = {
		...
		setTexture : function(default_image_url, default_image_type, imageUrl, image_type, flipX, flipY, image_updated_callback, attitude) {
			...
			var setTextureFunc = function(tex) {
				...
				var euler_correct = new THREE.Euler(THREE.Math.degToRad(attitude.Pitch), THREE.Math.degToRad(attitude.Yaw), THREE.Math.degToRad(attitude.Roll), "YXZ");
		...
		init : function(canvas) {
			...
			m_camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 1, 1100);
			...
			
			{
				...
				m_window_mode = true;
				m_video = new Image();
				m_video.onload = function(){
				    console.log("load complete");
				}
				m_video.src = "http://192.168.40.2:8080/?action=stream";
				//m_videoImage = document.createElement('canvas');
		...
		animate : function(elapsedTime) {
			if(m_window_mode) {
				if(stereoEnabled) {
					var sx=0,sy=0;
					var sa = m_video.width/m_video.height;
					var da = m_canvas.width/(m_canvas.height/2);
					if(sa > da) {
						sy = (m_video.height - m_video.width/da)/2;
					} else {
						sx = (m_video.width - m_video.height*da)/2;
					}
					m_context.drawImage(m_video, sx, sy, m_video.width - sx*2, m_video.height - sy*2, 0, 0, m_canvas.width, m_canvas.height/2);
					m_context.drawImage(m_video, sx, sy, m_video.width - sx*2, m_video.height - sy*2, 0, m_canvas.height/2, m_canvas.width, m_canvas.height/2);
				} else {
					var offset_x = (m_canvas.width - m_video.width) / 2;
					var offset_y = (m_canvas.height - m_video.height) / 2;
					m_context.drawImage(m_video, offset_x, offset_y, m_video.width, m_video.height);
				}
				return;
			}
			else if(m_video && m_videoTexture && m_video.duration > 0) {
				if(m_videoStart == 0) {
					m_videoStart = elapsedTime;
					m_video.currentTime = 0;
				}
				if (m_video.readyState === m_video.HAVE_ENOUGH_DATA) {
				    m_videoImageContext.drawImage(m_video, 0, 0);
				    m_videoTexture.needsUpdate = true;
	           		var videoElapsedTime = (elapsedTime - m_videoStart) / 1000.0;
		            m_video.currentTime = videoElapsedTime % m_video.duration;
				}
			}
			{// myAttitude
				var myAttitude_euler = new THREE.Euler(THREE.Math.degToRad(-myAttitude.Pitch), THREE.Math.degToRad(myAttitude.Yaw), THREE.Math.degToRad(myAttitude.Roll), "YXZ");
				...
				m_camera.target.applyQuaternion(quat2);
				m_camera.up.applyQuaternion(quat2);
