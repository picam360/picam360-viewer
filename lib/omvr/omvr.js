...
OMVR = (function() {
	...
	function sphereGeometry_p2v(fov, width, edge_r) {
		var bufferGeometry = new THREE.BufferGeometry();
		bufferGeometry.type = 'SphereBufferGeometry';
		...
		function get_position(k, i, j){
			...
			var r_edge1 = 1.0 - edge_r;
			var r_edge2 = sqrt(1.0*1.0 + edge_r*edge_r);
			if (r <= M_SQRT_2 && r > r_edge1) {
					...
					switch (scene.vertex_type) {
						...
						case "picam360map3d" :
							...
							if(m_geometry_cache[name]){
								...
								geometry = sphereGeometry_p2v(m_texture_fov, m_texture_height, m_edge_r);
							}else{
