	...
	function OMVR() {
		...
		var self = {
			...
			init : function(options) {
				const {canvas, devicePixelRatio, fxaa_enabled, callback} = options;
				m_canvas = canvas;
				m_fxaa_enabled = fxaa_enabled || false;
				...
				m_renderer = new THREE.WebGLRenderer({
					canvas : m_canvas,
					antialias : m_fxaa_enabled ? false : true,
				});
				...
				// texture
				var anisotropy = m_fxaa_enabled ? 0 : m_renderer.capabilities.getMaxAnisotropy();
				var filter = m_fxaa_enabled ? THREE.NearestFilter : THREE.LinearFilter;
...
//					//.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, width, height, 0, gl.RGB, gl.UNSIGNED_BYTE, img);
			...
			setModel : function(vertex_type, fragment_type, _scene) {
...

				{
					...
					if(m_fxaa_enabled && vertex_type == "picam360map3d" && fragment_type == "rgb"){
