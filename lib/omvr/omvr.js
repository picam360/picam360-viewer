			...
			for (var i = 0; i < width / 2; i+=BLOCK_SIZE) {
				for (var j = 0; j < width / 2; j+=BLOCK_SIZE) {
					...
					if(is_boundary(position_ary, [v1,v2,v3,v4])){
						var l_boundary = (j == 0) || is_boundary(
								position_ary, 
								get_rect_index(vertices, offset_idx, i, j - BLOCK_SIZE, BLOCK_SIZE));
						var t_boundary = (i == 0) || is_boundary(
								position_ary, 
								get_rect_index(vertices, offset_idx, i - BLOCK_SIZE, j, BLOCK_SIZE));
						var r_boundary = (j + BLOCK_SIZE == width / 2) || is_boundary(
								position_ary, 
								get_rect_index(vertices, offset_idx, i, j + BLOCK_SIZE, BLOCK_SIZE));
						var b_boundary = (i + BLOCK_SIZE == width / 2) || is_boundary(
								position_ary, 
								get_rect_index(vertices, offset_idx, i + BLOCK_SIZE, j, BLOCK_SIZE));
