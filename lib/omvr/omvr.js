...
// gl.UNSIGNED_BYTE, img);
			...
			animate : function(fov, quat) {
				if(m_updated_image){
					...
					m_texture_width = width;
					m_texture_height = height;
					m_texture_captured_time_ms = time;
					m_texture_updated_time_ms = new Date().getTime();
...
//				}
				{//latency
					...
					var latency = (now - m_texture_captured_time_ms) / 1000;
					if(m_latency == 0){
						m_latency = latency;
					} else {
						m_latency = m_latency * 0.9 + latency * 0.1;
					}
					if((m_animate_num % 200) == 0){
						var lag_time = now - m_texture_updated_time_ms;
						console.log("latency from capture to animate : " + m_latency + "ms, " + lag_time + "ms");
					}
				}
				m_animate_num++;
			},
