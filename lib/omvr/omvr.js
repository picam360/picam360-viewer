		...
		var self = {
			...
			init : function(options) {
				...
				
				{
					...
					addShadowedLight( 1, 1, 1, 0x888888, 1.35 );
...
// gl.UNSIGNED_BYTE, img);
			...
			animate : function(fov, quat) {
				...
				m_camera.fov = m_view_fov / m_camera.aspect;
				...
				if (stereoEnabled) {
					...
					camera_l.position.copy(new THREE.Vector3(100, 0, 0).applyQuaternion(view_quat));
					camera_l.aspect = w / h;
					camera_l.fov = m_view_fov / camera_l.aspect;
					...
					camera_r.position.copy(new THREE.Vector3(-100, 0, 0).applyQuaternion(view_quat));
					camera_r.aspect = w / h;
					camera_r.fov = m_view_fov / camera_r.aspect;
