		...
		function get_position(k, i, j){
			...
			if (r <= M_SQRT_2 && r > edge_r_s) {
				...
				if(gain_theta != 0.0){
					...
					if(abs(roll_diff) < M_PI_DIV_4){
						roll_diff = (1.0 - Math.pow(1.0 - abs(roll_diff)/M_PI_DIV_4, 1.0 + gain_theta*blend))*M_PI_DIV_4*(roll_diff>0.0?1.0:-1.0);
					}

					//var roll_diff_ratio = Math.abs(roll_diff)/M_PI_DIV_4;
					//var k_r = 0.7;
					//r = Math.pow((r-edge_r_s)/(M_SQRT_2-edge_r_s), 1.0 + k_r*roll_diff_ratio*roll_diff_ratio)*(M_SQRT_2-edge_r_s) + edge_r_s;
				}
...
//			for (var i = 0; i < width / 2; i+=BLOCK_SIZE) {
//				for (var j = 0; j < width / 2; j+=BLOCK_SIZE) {
//					if(is_boundary(position_ary, get_rect_index(vertices, offset_idx, i, j, BLOCK_SIZE))){
//						// boundary_map[i+"_"+j] = true;
//					}
//				}
//			}
