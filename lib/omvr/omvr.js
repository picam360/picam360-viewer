...
OMVR = (function() {
	...
	function sphereGeometry(fov, num_of_steps) {
		var bufferGeometry = new THREE.BufferGeometry();
	...
	function OMVR() {
		...
		var m_texture_captured_time = new Date().getTime();
		var m_texture_updated_time = new Date().getTime();
		var m_frame_num = 0;
		...
		var self = {
			...
			init : function(options) {
				...
				var shader_list = [{
					...
					url : "shader/picam360map3d_rgb_fxaa.frag?cache=no",
					shader : "picam360map3d_rgb_fxaa_fragment_shader"
				}, {
					url : "shader/picam360map3d_yuv_fxaa.frag?cache=no",
			...
			setTextureRawYuv : function(vertex_type, data, width, height, quat, fov, uuid) {
				...
				m_texture_updated_time = new Date().getTime();
			},
			...
			setTextureImage : function(vertex_type, img, quat, fov, uuid, time) {
				const {width,height} = img;
				self.setModel(vertex_type, "rgb");
				m_tex_quat = new THREE.Quaternion(quat._x, quat._y, quat._z, quat._w);
				m_texture_fov = fov;
			...
			setFrame: function(frame) {
				...
				if(max_ncc < 0.95){
					...
					if(m_frame_ary.length > MAX_TEXTURE_FRAMES){
						m_frame_ary = m_frame_ary.slice(m_frame_ary.length - MAX_TEXTURE_FRAMES);
					}
			...
			setFrameInfo: function(vertex_type, quat, fov, uuid) {
				...
				if(max_ncc < 0.95){
					...
					if(m_frame_info_ary.length > MAX_TEXTURE_FRAMES){
						m_frame_info_ary = m_frame_info_ary.slice(m_frame_info_ary.length - MAX_TEXTURE_FRAMES);
					}
...
//					//.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, width, height, 0, gl.RGB, gl.UNSIGNED_BYTE, img);
			...
			setModel : function(vertex_type, fragment_type, _view_fov, _scene) {
...

				{
					...
					switch (scene.vertex_type) {
						case "picam360map" :
						case "picam360map3d" :
							//geometry = sphereGeometry_p2v(view_fov, m_texture_height);
							geometry = sphereGeometry(view_fov, m_mesh_resolution);
							break;
					...
					if(m_fxaa_enabled && vertex_type == "picam360map3d"){
			...
			animate : function(fov, quat) {
				...
				if((m_frame_num % 200) == 0){
					var lag_time1 = new Date().getTime() - m_texture_captured_time;
					var lag_time2 = new Date().getTime() - m_texture_updated_time;
					console.log("animate lag time : " + lag_time1 + "ms, " + lag_time2 + "ms");
				}
				m_frame_num++;
			},
