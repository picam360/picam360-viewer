	...
	function OMVR() {
		...
		var self = {
			...
			setTextureRawYuv : function(vertex_type, data, width, height, quat, fov) {
				...
				gl.bindTexture(gl.TEXTURE_2D, m_renderer.properties.get(m_texture_y).__webglTexture);
				// m_renderer.setTexture2D(m_texture_y, 0);
				gl
					.texImage2D(gl.TEXTURE_2D, 0, gl.LUMINANCE, yDataPerRow, yRowCnt, 0, gl.LUMINANCE, gl.UNSIGNED_BYTE, yData);

				gl.bindTexture(gl.TEXTURE_2D, m_renderer.properties.get(m_texture_u).__webglTexture);
				// m_renderer.setTexture2D(m_texture_u, 0);
				gl
					.texImage2D(gl.TEXTURE_2D, 0, gl.LUMINANCE, uDataPerRow, uRowCnt, 0, gl.LUMINANCE, gl.UNSIGNED_BYTE, uData);

				gl.bindTexture(gl.TEXTURE_2D, m_renderer.properties.get(m_texture_v).__webglTexture);
				// m_renderer.setTexture2D(m_texture_v, 0);
				gl
			...
			set_vrmode : function(bln){
				m_renderer.vr.enabled = bln;
			},
			render : function(){
				if (stereoEnabled) {
