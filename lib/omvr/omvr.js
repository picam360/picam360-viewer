		...
		for (var k = 0; k < 4; k++) {
			...
			for (var i = 0; i < width / 2; i+=BLOCK_SIZE) {
				for (var j = 0; j < width / 2; j+=BLOCK_SIZE) {
					...
					if(is_boundary(position_ary, [v1,v2,v3,v4])){
						...
						for (var _i = 0; _i < BLOCK_SIZE; _i+=_BLOCK_SIZE) {
							for (var _j = 0; _j < BLOCK_SIZE; _j+=_BLOCK_SIZE) {
								var _v1 = vertices[_i/_BLOCK_SIZE + _offset_idx][_j/_BLOCK_SIZE + 1];
								var _v2 = vertices[_i/_BLOCK_SIZE + _offset_idx][_j/_BLOCK_SIZE];
								var _v3 = vertices[_i/_BLOCK_SIZE + 1 + _offset_idx][_j/_BLOCK_SIZE];
								var _v4 = vertices[_i/_BLOCK_SIZE + 1 + _offset_idx][_j/_BLOCK_SIZE + 1];
								indices.push(_v1, _v2, _v3);
								indices.push(_v3, _v4, _v1);
								//indices.push(v1, v2, v4);
								//indices.push(v2, v3, v4);
								if(_j==0){
									indices.push(v1, v2, _v2);
									indices.push(_v2, _v1, v1);
								}
								if(_i==0){
									indices.push(v2, v3, _v2);
									indices.push(_v3, _v2, v3);
								}
								if(_j+1==BLOCK_SIZE){
									indices.push(v3, v4, _v3);
									indices.push(_v4, _v3, v4);
								}
								if(_i+1==BLOCK_SIZE){
									indices.push(v4, v1, _v1);
					...
					switch (scene.vertex_type) {
						...
						case "picam360map3d" :
							...
							if(m_geometry_cache[name]){
								...
								geometry = sphereGeometry_p2v(m_texture_fov, m_texture_height);
							}else{
