...
void texture2D_4x4(vec4 out_pixels[16], float out_x[1], float out_y[1], float x, float y) { //x,y[0-1]
	...
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			...
			out_pixels[4*i + j] = texture2D_rgb(vec2(float(x2)/(texture_width - 1.0), float(y2)/(texture_height - 1.0)));
		}
	}
	...
	for(int i=0;i<4;i++){
		...
		for(int j=0;j<4;j++){
			...
			if(x2 == 0 || x2 == int(texture_width) - 1){
				xpixels[j] = texture2D_rgb(vec2(px, py));
				xpixels[j] += texture2D_rgb(vec2(px, 1.0 - py));
				xpixels[j] /= 2.0;
			}else if(y2 == 0 || y2 == int(texture_height) - 1){
				xpixels[j] = texture2D_rgb(vec2(px, py));
				xpixels[j] += texture2D_rgb(vec2(1.0 - px, py));
				xpixels[j] /= 2.0;
			}else{
				xpixels[j] = texture2D_rgb(vec2(px, py));
			}
...
vec4 texture2D_bicubic(vec2 uv)
{
    ...
    return g0(fuv.y) * (g0x * texture2D_rgb(p0)  +
                        g1x * texture2D_rgb(p1)) +
           g1(fuv.y) * (g0x * texture2D_rgb(p2)  +
                        g1x * texture2D_rgb(p3));
}

void main(void) {	
	if(resolution == 1.0){
		gl_FragColor = texture2D_rgb(tcoord);
	}else{
